...
public class HttpUtil {
    ...
    private static Response getPageByClient(String url, String ua, CookieStore cookieStore, Boolean useProxy) throws IOException {
        ...
        throw new UserException(String.format("访问网页失败 url[%s] ua[%s] userProxy[%s]", url, ua, useProxy));
    }
    ...
    public static String getPageSourceByClient(String url, String ua, CookieStore cookieStore, String charset, String nessaryString, boolean useProxy) {
            ...
            try {
                try {
                    if (useProxy) {
                        ...
                        if (StringUtils.isBlank(nessaryString)) {
                            return s;
                        }
                        if (s.contains(nessaryString)) {
                            return s;
                        } else {
                            throw new UserException(String.format("Error visit page [%s] via proxy [%s] no contains [%s]", url, proxy, nessaryString));
                        }
                    }
                } catch (Exception e) {
                }
        ...
        throw new UserException(String.format("访问网页失败 url[%s] ua[%s] userProxy[%s] charset[%s] nessaryString[%s]", url, ua, useProxy, charset, nessaryString));
    }
